...
namespace ProyectoHotel.Logica
{
    public class PersonaLogica
    {
        ...
        public List<Persona> ListarUsuario(int idPersona)
        {
            List<Persona> Lista = new List<Persona>();
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                ...
                {
                    StringBuilder sb = new StringBuilder();
                    sb.AppendLine("select p.IdPersona, p.TipoDocumento, p.Documento, p.Nombre, p.Apellido, p.Correo, p.Clave, tp.IdTipoPersona, tp.Descripcion, p.Estado");
                    sb.AppendLine("from persona p");
                    sb.AppendLine("inner join TIPO_PERSONA tp on tp.IdTipoPersona = p.IdTipoPersona");
                    sb.AppendLine("where p.IdPersona = @IdPersona");
        ...
        public bool ActualizarClave(int id, string nuevaClave)
        {
            bool respuesta = true;
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    SqlCommand cmd = new SqlCommand("update PERSONA set Clave = @nuevaClave where IdPersona = @id", oConexion);
